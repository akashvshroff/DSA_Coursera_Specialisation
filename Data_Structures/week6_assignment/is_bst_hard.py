...
class CheckBST:
    ...
    def is_binary_search_tree(self, cur, min_, max_):
        ...
        in this scenario, equal keys are allowed but are restricted to the right
        sub-tree and therefore the conditional has been adapted accordingly.

        The conditional is min_ <= key < max_ since for a right-sub tree, the
        root is the min value and for a left, the root is the max value.
        """
        if cur == -1:
            return True
        if not min_ <= self.key[cur] < max_:  # duplicate in right not left
            return False
